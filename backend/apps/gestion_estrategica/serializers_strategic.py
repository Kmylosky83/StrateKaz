...
Este modulo contiene los serializers para:
...
NOTA: Este archivo fue movido desde apps.core.serializers_strategic
para eliminar dependencias circulares donde core importaba de gestion_estrategica.
"""
...
# Modelos de gestion_estrategica - imports relativos (TAB = Django App)
...
class SignPolicySerializer(serializers.Serializer):
    """Serializer para firmar la Politica Integral"""

    confirm = serializers.BooleanField(
        help_text='Confirmar la firma digital de la politica'
    )
...
class StrategicObjectiveListSerializer(serializers.ModelSerializer):
    """Serializer para listado de Objetivos Estrategicos"""
...
class StrategicObjectiveDetailSerializer(serializers.ModelSerializer):
    """Serializer detallado de Objetivo Estrategico"""
...
class StrategicObjectiveCreateSerializer(serializers.ModelSerializer):
    """Serializer para crear Objetivo Estrategico"""
    ...
    def validate_code(self, value):
        ...
        if plan and StrategicObjective.objects.filter(plan_id=plan, code=value).exists():
            raise serializers.ValidationError('Este codigo ya existe en el plan')
        return value
...
class StrategicObjectiveUpdateSerializer(serializers.ModelSerializer):
    """Serializer para actualizar Objetivo Estrategico"""
...
class StrategicPlanListSerializer(serializers.ModelSerializer):
    """Serializer para listado de Planes Estrategicos"""
...
class StrategicPlanDetailSerializer(serializers.ModelSerializer):
    """Serializer detallado de Plan Estrategico"""
...
class StrategicPlanCreateSerializer(serializers.ModelSerializer):
    """Serializer para crear Plan Estrategico"""
...
class StrategicPlanUpdateSerializer(serializers.ModelSerializer):
    """Serializer para actualizar Plan Estrategico"""
...
class ApprovePlanSerializer(serializers.Serializer):
    """Serializer para aprobar Plan Estrategico"""

    confirm = serializers.BooleanField(
        help_text='Confirmar la aprobacion del plan'
    )

    def validate_confirm(self, value):
        if not value:
            raise serializers.ValidationError('Debe confirmar la aprobacion')
        return value
...
class SystemModuleListSerializer(serializers.ModelSerializer):
    """Serializer para listado de Modulos del Sistema"""
...
class SystemModuleDetailSerializer(serializers.ModelSerializer):
    """Serializer detallado de Modulo del Sistema"""
...
class SystemModuleCreateSerializer(serializers.ModelSerializer):
    """Serializer para crear Modulo del Sistema"""

    dependency_ids = serializers.ListField(
        child=serializers.IntegerField(),
        write_only=True,
        required=False,
        help_text='IDs de modulos de los que depende'
    )
    ...
    def validate_code(self, value):
        if SystemModule.objects.filter(code=value).exists():
            raise serializers.ValidationError('Este codigo ya existe')
        return value
...
class SystemModuleUpdateSerializer(serializers.ModelSerializer):
    """Serializer para actualizar Modulo del Sistema"""
...
class ToggleModuleSerializer(serializers.Serializer):
    """Serializer para activar/desactivar modulo"""
...
# =============================================================================
# SERIALIZERS PARA SISTEMA DINAMICO DE MODULOS
# =============================================================================
...
class ModuleTabSerializer(serializers.ModelSerializer):
    """Serializer para tabs de un modulo"""
...
class SystemModuleTreeSerializer(serializers.ModelSerializer):
    """Serializer para modulos con arbol completo (tabs y secciones)"""
    ...
    def get_color(self, obj):
        """Retorna color efectivo (asignado o por categoria)"""
...
class ModulesTreeSerializer(serializers.Serializer):
    """Serializer para el arbol completo de modulos"""
...
class ToggleSectionSerializer(serializers.Serializer):
    """Serializer para activar/desactivar una seccion"""
    is_enabled = serializers.BooleanField()


# Serializers compactos para el Sidebar (sin anidar todo)
# =============================================================================
# TABS Y SECCIONES DE MODULOS
# =============================================================================

class TabSectionDetailSerializer(serializers.ModelSerializer):
    """Serializer para Secciones de Tabs con detalle"""
...
class ModuleTabDetailSerializer(serializers.ModelSerializer):
    """Serializer para Tabs de Modulos con detalle"""
    module_name = serializers.CharField(source='module.name', read_only=True)
    sections = TabSectionDetailSerializer(many=True, read_only=True)
...
class ModuleTabCreateSerializer(serializers.ModelSerializer):
    """Serializer para crear Tabs de Modulos"""
...
class SidebarModuleSerializer(serializers.Serializer):
    """
    Serializer para modulos del Sidebar con estructura jerarquica.
    ...
    def get_route(self, obj):
        """
        Genera la ruta del modulo basada en su codigo.
        Las categorias (6 niveles) no tienen ruta.
        """
    ...
    def get_is_category(self, obj):
        """
        Determina si es una categoria o un modulo navegable.
        Las 6 categorias (ESTRATEGICO, MOTOR, INTEGRAL, MISIONAL, APOYO, INTELIGENCIA)
        son grupos que contienen modulos.
        """
    ...
    def get_children(self, obj):
        ...
        - Si es categoria: retorna los modulos de esa categoria
        - Si es modulo: retorna los tabs habilitados como hijos
        """
        if isinstance(obj, dict):
            # Es una categoria, los hijos son los modulos
            modules = obj.get('modules', [])
            return SidebarModuleSerializer(modules, many=True).data

        # Es un modulo, los hijos son los tabs habilitados
        tabs = obj.get_enabled_tabs() if hasattr(obj, 'get_enabled_tabs') else []
        if not tabs:
            return None

        # Convertir tabs a estructura de navegacion
        children = []
        for tab in tabs:
            child = {
                'code': tab.code,
...
class ModulesTreeFullSerializer(serializers.Serializer):
    """Serializer para arbol completo de modulos"""
