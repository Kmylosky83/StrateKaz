...
# --- Juego SST: Los Héroes de la Seguridad ---
...
class GameLevel(BaseCompanyModel):
    """
    ...
    # Mapa (referencia al archivo Tiled JSON en public/game/maps/)
...
class GameQuizQuestion(BaseCompanyModel):
    """
...
class GameProgress(BaseCompanyModel):
    """
    ...
    def __str__(self):
        return f"{self.colaborador.get_nombre_corto()} - Nivel {self.nivel_actual} ({self.xp_total} XP)"
...
class GameSession(BaseCompanyModel):
    """
    ...
    def __str__(self):
        ...
        return f"{estado} {self.colaborador.get_nombre_corto()} - Nivel {self.nivel.numero_nivel} ({self.puntaje} pts)"
