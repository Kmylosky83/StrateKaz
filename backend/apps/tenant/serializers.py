...
class TenantCreateSerializer(serializers.ModelSerializer):
    ...
    IMPORTANTE: El frontend envía solo el subdomain (ej: "miempresa").
    ...
    def validate(self, attrs):
        ...
        if not subdomain and not domain:
            raise serializers.ValidationError(
                {'subdomain': 'El subdominio es requerido.'}
            )

        # Extraer subdomain limpio
        if subdomain:
            clean_subdomain = subdomain.strip().lower()
        else:
            # Legacy: extraer subdomain del domain completo
            clean_subdomain = domain.split('.')[0].strip().lower()

        # Validar formato del subdomain
        import re
        if not re.match(r'^[a-z0-9]+$', clean_subdomain):
            raise serializers.ValidationError(
                {'subdomain': 'Solo letras minúsculas y números.'}
            )

        # Construir dominio completo con PLATFORM_DOMAIN del backend
        platform_domain = getattr(settings, 'PLATFORM_DOMAIN', 'stratekaz.com')
        full_domain = f'{clean_subdomain}.{platform_domain}'
