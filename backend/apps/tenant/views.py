...
class TenantUserViewSet(viewsets.ModelViewSet):
    ...
    def tenants(self, request):
        ...
        Para superadmins: retorna TODAS las empresas activas (para monitoreo/soporte).
        Para usuarios normales: retorna solo las empresas asignadas.
        """
        email = request.user.email
        try:
            tenant_user = TenantUser.objects.prefetch_related(
            ...
            # Si es superadmin, retornar TODAS las empresas activas
            if tenant_user.is_superadmin:
                all_tenants = Tenant.objects.filter(is_active=True).order_by('name')
                tenants_data = []
                for tenant in all_tenants:
                    # Verificar si tiene acceso explícito para obtener el rol
                    access = tenant_user.accesses.filter(
                    ...
                    tenants_data.append({
                        ...
                        'role': access.role if access else 'admin',  # Superadmin tiene rol admin
                        'is_active': True,
                    })
            else:
                # Usuario normal: solo empresas asignadas
    ...
    def select(self, request):
        ...
        Para superadmins: pueden acceder a cualquier empresa activa.
        Para usuarios normales: solo empresas asignadas.
        """
        tenant_id = request.data.get('tenant_id')
        if not tenant_id:
            return Response(
        ...
        if tenant_user.is_superadmin:
            ...
            # Usuario normal: verificar acceso explícito
            try:
