...
export function FirmantesEditor({ firmantes, onChange, plantillaId }: FirmantesEditorProps) {
  ...
  const handleAdd = () => {
    ...
    const nextRol = suggestedRoles[Math.min(firmantes.length, suggestedRoles.length - 1)];
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  <Select
                    ...
                    onChange={(e) =>
                      handleFieldChange(index, 'rol_firma', e.target.value)
                    }
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      handleFieldChange(index, 'cargo_code', e.target.value)
                    }
                  >
                    ...
                    {isLoadingCargos ? (
                      ...
                      cargos?.map((c) => (
                        <option key={c.id} value={c.extra?.code ?? ''}>
                          {c.label}
                        </option>
                      ))
                    )}
