...
describe('Sistema Documental - API Tests', () => {
  ...
  // ==================== TEST 2: documentoApi.create ====================
  describe('documentoApi.create', () => {
    it('debe crear un nuevo documento correctamente', async () => {
      const mockDocumento = {
        id: 1,
        codigo: 'MAN-001',
      ...
      const createData = {
        titulo: 'Manual de Seguridad',
        tipo_documento: 1,
        contenido: 'Contenido del manual...',
      };

      vi.mocked(apiClient.post).mockResolvedValueOnce({ data: mockDocumento });
  ...
  // ==================== TEST 3: documentoApi.aprobar ====================
  describe('documentoApi.aprobar', () => {
    it('debe aprobar un documento correctamente', async () => {
      const mockDocumentoAprobado = {
        id: 1,
        codigo: 'MAN-001',
        titulo: 'Manual de Seguridad',
        estado: 'APROBADO',
      ...
      vi.mocked(apiClient.post).mockResolvedValueOnce({ data: mockDocumentoAprobado });
      ...
      expect(apiClient.post).toHaveBeenCalledWith('/api/hseq/documentos/documentos/1/aprobar/');
      expect(result.current.data).toEqual(mockDocumentoAprobado);
    });

    it('debe manejar errores al aprobar documento', async () => {
      vi.mocked(apiClient.post).mockRejectedValueOnce(new Error('No autorizado'));
  ...
  // ==================== TEST 4: firmaDocumentoApi.firmar ====================
  describe('firmaDocumentoApi.firmar', () => {
    it('debe firmar un documento correctamente', async () => {
      const mockFirma = {
        id: 1,
        documento: 1,
        tipo_firma: 'APROBACION',
        estado: 'FIRMADO',
        fecha_firma: '2025-12-26T10:30:00Z',
        firmante: 1,
        cargo_firmante: 'Gerente HSEQ',
      };

      const firmaData = {
        firma_digital: 'data:image/png;base64,iVBOR...',
        comentarios: 'Aprobado',
      };

      vi.mocked(apiClient.post).mockResolvedValueOnce({ data: mockFirma });

      const { result } = renderHook(() => useFirmarDocumento(), {
        wrapper: createWrapper(queryClient),
      });

      result.current.mutate({ id: 1, datos: firmaData });

      await waitFor(() => expect(result.current.isSuccess).toBe(true));

      expect(apiClient.post).toHaveBeenCalledWith('/api/hseq/documentos/firmas/1/firmar/', firmaData);
      expect(result.current.data).toEqual(mockFirma);
    });

    it('debe validar que la firma requiera datos obligatorios', async () => {
      vi.mocked(apiClient.post).mockRejectedValueOnce(
        new Error('Firma digital requerida')
      );
      ...
      result.current.mutate({ id: 1, datos: {} });
