...
import { TranscribirTalonarioModal } from './TranscribirTalonarioModal';
...
export const VoucherDetailModal = ({ voucher, isOpen, onClose }: VoucherDetailModalProps) => {
  ...
  const [showTalonarioModal, setShowTalonarioModal] = useState(false);
  ...
  // Descarga el PDF 80mm del voucher con JWT (apiClient) y lo abre en un
  // popup. No se puede usar window.open(url) directo porque el backend
  // exige Bearer token y window.open no adjunta headers (H-SC-E2E-04).
  // 2026-04-27: refactor a PDF (antes HTML inline con auto-print JS).
  ...
  // PDF carta — descarga formal del voucher para archivo.
  const handleDownloadCarta = async () => {
    try {
      const { data } = await apiClient.get<Blob>(
        `/supply-chain/recepcion/vouchers/${voucher.id}/pdf-carta/`,
        { responseType: 'blob', headers: { Accept: 'application/pdf' } }
      );
      const blobUrl = URL.createObjectURL(data);
      const a = document.createElement('a');
      a.href = blobUrl;
      a.download = `voucher-${voucher.id}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      setTimeout(() => URL.revokeObjectURL(blobUrl), 60000);
    } catch {
      toast.error('No se pudo descargar el PDF carta');
    }
  };
  ...
  const footer = (
    <>
      ...
      <Button variant="outline" onClick={handleDownloadCarta}>
  ...
  // Botón "Transcribir talonario manual" — visible solo cuando aplica.
  const canTranscribirTalonario =
    voucher.modalidad_entrega === 'RECOLECCION' &&
    Boolean(voucher.ruta_recoleccion) &&
    voucher.estado === 'PENDIENTE_QC' &&
    (voucher.vouchers_recoleccion_info?.length ?? 0) === 0;
    ...
    <>
      ...
      >
        <div className="space-y-6">
          ...
          {/* H-SC-RUTA-02 (refactor 2): M2M con N vouchers de recolección.
              Sólo aplica a modalidad RECOLECCION (entrega DIRECTO o
              TRANSPORTE_INTERNO no usan vouchers de ruta). */}
          {voucher.modalidad_entrega === 'RECOLECCION' && (
            <Card>
              <div className="p-4">
                ...
                {canTranscribirTalonario && (
                  <div className="mb-3">
                    <Button variant="secondary" onClick={() => setShowTalonarioModal(true)}>
                      <FileText className="w-4 h-4 mr-2" />
                      Transcribir talonario manual
                    </Button>
                  </div>
                )}

                <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                  Marca los vouchers de recolección que se consolidaron en esta recepción. El
                  inventario YA entró — los vouchers asocian para liquidar a cada productor por
                  separado.
                </p>
      ...
      {showTalonarioModal && voucher.ruta_recoleccion && (
        <TranscribirTalonarioModal
          isOpen={showTalonarioModal}
          onClose={() => setShowTalonarioModal(false)}
          voucherRecepcionId={voucher.id}
          rutaId={voucher.ruta_recoleccion}
          fechaRecoleccion={voucher.fecha_viaje}
          onSuccess={() => {
            qc.invalidateQueries();
          }}
        />
