...
export const LoginPage = () => {
  ...
  // Si ya hay sesión activa (tokens + Zustand + user cargado), redirigir.
  // Esto cubre: navegación manual a /login, PWA abre en /login.
  // NOTA: login() hace set() atómico al final, así que isAuthenticated=true
  // SOLO se pone cuando user ya está cargado. No hay race condition.
  // Pero este effect NO navega post-login — eso lo hace handlePostLoginFlow().
  // Solo navega cuando la sesión ya existía ANTES de montar este componente.
  const [loginHandled, setLoginHandled] = useState(false);
  useEffect(() => {
    // Si el login acaba de completarse vía onSubmit, no competir con handlePostLoginFlow
  ...
  }, [isAuthenticated, loginHandled, navigate]);
  ...
  const onSubmit = async (data: LoginFormData) => {
    setIsLoading(true);
    setLoginHandled(true); // Bloquear useEffect de sesión previa
    try {
      // login() es atómico: API call → selectTenant → getProfile → set() UNA vez.
      // No hay re-render intermedio, no hay race conditions.
      await login(data);

      // Login exitoso — handlePostLoginFlow decide navegación
      await handlePostLoginFlow();
    } catch (error: unknown) {
      setLoginHandled(false); // Permitir que useEffect funcione de nuevo
