 ...
 * Synchronized with frontend NetworkBackground for visual consistency
 *
...
const NetworkAnimation: React.FC<NetworkAnimationProps> = ({ isVisible, particleCount }) => {
  ...
  const particles = useMemo<Particle[]>(() => {
    const temp: Particle[] = [];
    const pink = new THREE.Color(PARTICLE_CONFIG.brandColor);
    const white = new THREE.Color('#ffffff');

    for (let i = 0; i < particleCount; i++) {
      // Color mix: ~70% Pink, ~30% White for visual variety
      const isPink = Math.random() > (1 - PARTICLE_CONFIG.primaryColorRatio);
      const color = isPink ? pink : white;
  ...
  return (
    <>
      ...
      <lineSegments ref={linesRef} geometry={linesGeometry}>
        <lineBasicMaterial
          color={PARTICLE_CONFIG.brandColor}
          transparent
          opacity={0.12}
          depthWrite={false}
        />
...
/**
 * ThreeBackground Component
 * The main container for the 3D background.
 * Renders a "Connected Nodes" network animation in brand pink (and white).
 */
